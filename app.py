...
# Title
st.markdown("<h1 class='main-header'>🚀 AIOps Dashboard - System Monitoring & Analytics</h1>", unsafe_allow_html=True)
...
with tab1:
    ...
    st.metric("🌐 Network I/O", 
              f"↑{net.bytes_sent/1e6:.1f}MB ↓{net.bytes_recv/1e6:.1f}MB")
    
    # Line 8: System status
    status = "🟢 Healthy" if cpu_usage < 80 and ram.percent < 85 else "🔴 Warning"
...
with tab3:
    ...
    if uploaded_file is not None:
        try:
            ...
            if not uploaded_df.empty:
                logs_df = uploaded_df
                st.success(f"✅ Loaded {len(logs_df)} log entries from uploaded file")
    ...
    with filter_col1:
        # FIXED: Get unique values and ensure defaults exist
        available_levels = logs_df['level'].unique().tolist()
        default_levels = [lvl for lvl in ['ERROR', 'WARNING'] if lvl in available_levels]
        selected_levels = st.multiselect(
            "Log Level",
            options=available_levels,
            default=default_levels if default_levels else available_levels[:2]
        )
    
    with filter_col2:
        available_services = logs_df['service'].unique().tolist()
        default_services = available_services[:3] if len(available_services) >= 3 else available_services
    ...
    # Display logs
    st.subheader(f"📋 Log Entries ({len(filtered_logs)} found)")
...
with tab4:
    ...
    for idx, row in anomalies_df.iterrows():
        if row['is_anomaly']:
            # FIXED: Use separate lists instead of list of tuples
            log_types_list = [
            ...
            chosen_idx = np.random.choice(len(log_types_list), p=probabilities)
            log_level, log_msg = log_types_list[chosen_idx]
            
            anomaly_logs.append({
                'timestamp': row['timestamp'],
                'level': log_level,
                'service': 'system-monitor',
                'message': f"{log_msg} - Value: {row['metric_value']:.1f}%",
    ...
    with col1:
        st.subheader("📈 Metrics with Detected Anomalies")
        ...
        # Add anomaly points
        anomaly_points_df = anomalies_df[anomalies_df['is_anomaly']]
        fig_anomalies.add_trace(go.Scatter(
            x=anomaly_points_df['timestamp'],
            y=anomaly_points_df['metric_value'],
            mode='markers',
            name='Anomaly',
            marker=dict(
                color='red',
        ...
        with col_stat1:
            st.metric("Total Anomalies", len(anomaly_points_df))
        
        with col_stat2:
            st.metric("Anomaly Rate", f"{(len(anomaly_points_df)/len(anomalies_df)*100):.1f}%")
        
        with col_stat3:
            avg_anomaly_val = anomaly_points_df['metric_value'].mean() if not anomaly_points_df.empty else 0
            st.metric("Avg Anomaly Value", f"{avg_anomaly_val:.1f}%")
    
    with col2:
        st.subheader("📋 Correlated Log Events")
        ...
        if not anomaly_logs_df.empty:
            ...
            if len(anomaly_logs_df) > 0:
                ...
                <ul>
                    <li><strong>{len(anomaly_points_df)} anomalies</strong> detected in metrics</li>
    ...
    if st.button("🎯 Simulate Real-time Anomaly"):
